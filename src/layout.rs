...
fn shape_with_fallback(
    ...
    // Phase 3: final fallback - try all loaded fonts for coverage.
    // Fonts from the same source file (e.g. TTC faces) are deduplicated
    // because they share the same cmap.
    let all_keys = font_system.all_font_keys();
    let needed: Vec<char> = text.chars().collect();
    let mut result: Vec<ShapedRun> = Vec::new();
    let mut remaining: Vec<(usize, usize)> = vec![(run_start, run_start + text.len())];
    let mut seen_sources: Vec<String> = Vec::new();

    // Quick heuristic: detect if the text contains non-Latin codepoints
    // so we can skip fonts that clearly don't cover them without reading data.
    let has_cjk = needed.iter().any(|c| {
        let cp = *c as u32;
        (0x2E80..=0x9FFF).contains(&cp) // CJK Radicals through Unified Ideographs
    ...
    for key in all_keys {
        ...
        // Deduplicate fonts from same source file (e.g. TTC with multiple faces).
        let face_info = match font_system.db.face(key.0) {
            Some(info) => info,
            None => continue,
        };
        let source_key = source_to_key(&face_info.source);
        if seen_sources.contains(&source_key) {
            continue;
        }
        seen_sources.push(source_key);

        // Family-name heuristic: skip fonts that clearly don't cover CJK
        // without reading their font data (avoiding disk I/O for 95% of fonts).
        if has_cjk {
            let name = face_info
                .families
                .first()
                ...
                .unwrap_or_default();
            let likely_cjk = name.contains("cjk")
                || name.contains("noto")
        ...
        // Lightweight cmap pre-filter using ttf_parser.
        // Only fonts that cover at least one needed codepoint will have
        // their data loaded and a rustybuzz::Face created.
        let covers_any = font_system
            .db
            .with_face_data(key.0, |data, index| {
                ttf_parser::Face::parse(data, index)
                    .ok()
                    .is_some_and(|face| needed.iter().any(|&c| face.glyph_index(c).is_some()))
            })
            .unwrap_or(false);
        ...
        let mut next_remaining: Vec<(usize, usize)> = Vec::new();

        for &(range_start, range_end) in &remaining {
            let sub_start = range_start - run_start;
            let sub_end = range_end - run_start;
            let subtext = &text[sub_start..sub_end];

            let (glyphs, has_notdef) = {
                let face = match font_system.get_or_create_face(key) {
                    Some(f) => f,
                    None => continue,
                };
                let g = shape_text(face, subtext, font_size, direction);
                let h = g.iter().any(|g| g.glyph_id == 0);
                (g, h)
            };
            ...
            let ranges = coverage_ranges(subtext, &glyphs);
            for (r_start, r_end, covered) in ranges {
                if r_start >= r_end {
                    continue;
                }
                if covered {
                    let piece = &subtext[r_start..r_end];
                    let sub_glyphs = {
                        let face = font_system.get_or_create_face(key).unwrap();
                        shape_text(face, piece, font_size, direction)
                    };
                    let mut adjusted = sub_glyphs;
                    for g in &mut adjusted {
                        g.cluster += (range_start + r_start) as u32;
                    }
                    result.push(ShapedRun {
                        glyphs: adjusted,
                        font_key: key,
                    ...
                    next_remaining.push((range_start + r_start, range_start + r_end));
                }
            }
        }
...
/// Produces a deduplication key for a font source.
/// Fonts from the same source file (e.g. different faces in a TTC) share
/// the same cmap, so we only need to check coverage once per source.
fn source_to_key(source: &fontdb::Source) -> String {
    match source {
        fontdb::Source::File(path) | fontdb::Source::SharedFile(path, _) => {
            path.to_string_lossy().to_string()
        }
        fontdb::Source::Binary(arc) => {
            format!("{:p}", arc.as_ref().as_ref() as *const [u8])
        }
    }
}
